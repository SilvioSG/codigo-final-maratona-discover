...
const Storage = {
    ...
    set(transactions) {
        localStorage.setItem("dev.fincances:transactions",
    ...
    incomes() {
        ...
        Transaction.all.forEach(transaction => {
            if (transaction.amount >0 ) {
                //Somar a uma variavel e retonar ele
...
const DOM = {
    ...
    addTransaciton(transaction, index){
    ...
    innerHTMLTransaction(transaction, index) {
        ...
        const aumont= Utils.formatCurrency(transaction.amount)

        const html = `
             <td class="description">${transaction.description}</td>
             <td class="${CSSclass}">${aumont}</td>
...
const Form = {
    ...
    validateField () { //Validação dos campos
...
const App = {
    init () {
        ...
        Transaction.all.forEach(DOM.addTransaciton) 
